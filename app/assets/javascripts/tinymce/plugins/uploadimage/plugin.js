	...
	tinymce.create('tinymce.plugins.UploadImage', {
		UploadImage: function(ed, url) {
			...
			function showDialog() {
				win = editor.windowManager.open({
					...
					body: [
						...
						{type: 'textbox', name: 'image', label: ed.translate('Choose an image'), subtype: 'file'},
				...
				form.appendChild(createElement('input', {type: 'hidden', name: 'hint', value: ed.getParam("uploadimage_hint", "")}));
				form.appendChild(createElement('input', {type: 'hidden', name: 'constrain', value: true}));
				...
				for(var i = 0; i < inputs.length; i++) {
					...
					if(ctrl.tagName.toLowerCase() == 'input' && ctrl.type != "hidden" && ctrl.type != "checkbox") {
						if(ctrl.type == "file") {
							ctrl.name = "image";
						...
						} else if (ctrl.getAttribute('aria-label') == "Width") {
							ctrl.name = "width";
						} else if (ctrl.getAttribute('aria-label') == "Height") {
							ctrl.name = "height";
						} else {
							ctrl.name = "alt";
						}
			...
			function insertImage() {
				if(getInputValue("image") == "") {
					return handleError('You must choose a image');
				}
				...
				*/
				...
				var el = win.getEl();
				var body = document.getElementById(el.id + "-body");
				var checkbox = body.querySelector("[role=checkbox]");
				if (checkbox.getAttribute('aria-checked') == "true") {
					document.getElementsByName('constrain')[0].value = true;
				} else {
					document.getElementsByName('constrain')[0].value = false;
				}
				form.submit();
			}
